...
def collect_teachers_predictions(predictions_folder):
    ...
    nb_samples = len(ex_teacher)
    nb_classes = 10 # here, hardcoded

    # Array containing for each teacher, the predicted class (class ID) for each data sample
    teachers_predictions = np.zeros((nb_teachers, nb_samples), dtype=np.int8)
    ...
    for teacher_id, teacher_preds_file_i in enumerate(predictions_files):
        predictions_teacher_i = np.load( os.path.join(predictions_folder, teacher_preds_file_i) )
